    ...
    return (
        <Layout {...props}>
            <section className="post">
                ...
                <div>
                    <h2>Reviews</h2>
                    {_.get(props, 'pageContext.frontmatter.review', null).map(({ title, rating, message }, index) => (
                        <div key={index}>
                            <h3>{title}</h3>
                            <p>{rating}</p>
                            <p>{message}</p>
                ...
                <div>
                    ...
                    <a href={`https://picsoung.typeform.com/to/SL09LYJ3#product_name=${props.pageContext.frontmatter.title}&product_id=xxxxx`}> Review</a>
                </div>
            </section>
        </Layout>
    );
